...
shapes_2d = ['rectangle', 'circle', 'triangle', 'pentagon', 'star']  # removed hexagon, reordered for common ones
...
def get_projected_center(shape, w, h):
    ...
    elif isinstance(shape, dict) and 'pos' in shape:
        return shape['pos']
    return (0, 0)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_lm, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            fingers = sum(1 for tip in [8,12,16,20] if hand_lm.landmark[tip].y < hand_lm.landmark[tip-3].y - 0.035)
    ...
    if current_mode == 0:
        draw_pos = None
        if active_index_pos:
            ix, iy = active_index_pos
            if smoothed_x is None:
                smoothed_x, smoothed_y = ix, iy
            else:
                smoothed_x = alpha * smoothed_x + (1 - alpha) * ix
                smoothed_y = alpha * smoothed_y + (1 - alpha) * iy
            draw_pos = (int(smoothed_x), int(smoothed_y))

            if 1 <= active_fingers <= 3:
                if last_point and draw_pos:
                    cv2.line(canvas, last_point, draw_pos, colors[current_color], 8, cv2.LINE_AA)
                last_point = draw_pos
            else:
                last_point = None
            ...
            if active_fingers >= 4:
                if draw_pos:
                    cv2.circle(canvas, draw_pos, 60, (0,0,0), -1)
        else:
    ...
    # ── Shape modes ─────────────────────────────────────────────────
        ...
        if spawn_cooldown == 0 and not grabbed_shape:
            ...
            # Double pinch → rectangle (2D) or cube (3D)
            if left_pinching and right_pinching and left_pinch_pos is not None and right_pinch_pos is not None:
                ...
                if 80 < dist < 520:
                    ...
                    if current_mode == 1:
                        new_shape = {'type': 'rectangle', 'pos': pos, 'color': colors[current_color], 'scale': 1.0}
                    elif current_mode == 2:
                        new_shape = Shape3D('cube', pos, colors[current_color])

            # Left pinch + right fingers → different per mode
                ...
                if current_mode == 1:
                    # 2D: 2→circle, 3→triangle, 4→pentagon, 5→star
                    shape_map_2d = {2: 'circle', 3: 'triangle', 4: 'pentagon', 5: 'star'}
                    shape_type = shape_map_2d.get(right_fingers, 'star')  # default star if >5
                    new_shape = {'type': shape_type, 'pos': pos, 'color': colors[current_color], 'scale': 1.0}

                elif current_mode == 2:
                    # 3D: 2→pyramid, 3→cylinder, 4→cone, 5→sphere
                    shape_map_3d = {2: 'pyramid', 3: 'cylinder', 4: 'cone', 5: 'sphere'}
                    shape_type = shape_map_3d.get(right_fingers, 'sphere')
                    new_shape = Shape3D(shape_type, pos, colors[current_color])
        ...
        if grabbed_shape is not None:
            ...
            if pinch_pos is not None:
                ...
                if current_mode == 2 and isinstance(grabbed_shape, Shape3D):
                    target = screen_to_world(desired[0], desired[1], grabbed_shape.position[2], w, h)
                    alpha_move = 0.93
                    grabbed_shape.position = alpha_move * target + (1 - alpha_move) * grabbed_shape.position
                elif current_mode == 1 and isinstance(grabbed_shape, dict):
    ...
    if current_mode in (1, 2) and objects:
        ...
        if current_mode == 1 and isinstance(obj, dict) and 'type' in obj:
            draw_2d_shape(frame, obj)
        elif current_mode == 2 and isinstance(obj, Shape3D):
